...
static VERTEX_3D_EFFECT v[4]={ {D3DXVECTOR3(-0.5,0.5,0),D3DCOLOR_RGBA(255,255,255,255) ,D3DXVECTOR2 (0.0f,0.0f)  },
...
void UpdateEffect(void)
{
	...
	for( int i = 0 ; i<EFFECT_MAX; i++)
	{
		if(g_Effects[i].bEffect)
		{
			...
			v[0].color=D3DCOLOR_RGBA(10,255,255,g_Effects[i].nAlpha);
			v[1].color=D3DCOLOR_RGBA(10,255,255,g_Effects[i].nAlpha);
			v[2].color=D3DCOLOR_RGBA(10,255,255,g_Effects[i].nAlpha);
			v[3].color=D3DCOLOR_RGBA(10,255,255,g_Effects[i].nAlpha);
	
			 CreateVertex_ef();
			 

			D3DXMatrixTranslation( &g_Effects[i].g_mtxWorld_k, g_Effects[i].vPos.x ,g_Effects[i].vPos.y  ,g_Effects[i].vPos.z);
			D3DXMatrixScaling(&g_Effects[i].g_mtxWorld,0.1,0.1,1);
...
void DrawEffect(void) 
{
	...
	for(int i = 0  ; i<EFFECT_MAX ; i++)
	{
		...
		if(!g_Effects[i].bEffect)continue;



		//float age = g_nFrameCount/g_Effects[i].nBirth;
		//float alpha = 1.0f-age/(float)g_Effects[i].nLimit;
