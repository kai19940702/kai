...
// グローバル変数
//*****************************************************************************
LPDIRECTINPUT8			g_pInput = NULL;					// DirectInputオブジェクトへのポインタ		<dinput.h>
LPDIRECTINPUTDEVICE8	g_pDevKeyboard = NULL;				// 入力デバイス(キーボード)へのポインタ		<dinput.h>
...
HRESULT InitKeyboard(HINSTANCE hInstance, HWND hWnd)
{
	...
	// デバイスの作成
	if(FAILED(g_pInput->CreateDevice(GUID_SysKeyboard, &g_pDevKeyboard, NULL))) // 第一引数にマウス、パッド用にできる
	{
...
void UpdateKeyboard(void)
{
	...
	if(SUCCEEDED(g_pDevKeyboard->GetDeviceState(sizeof(aKeyState), aKeyState)))
	{
		...
		{
			// キートリガー・リリース情報を生成
			g_aKeyStateTrigger[nCnKey] = (g_aKeyState[nCnKey] ^ aKeyState[nCnKey]) & aKeyState[nCnKey];	// キーが押されたばっかかどうか調べる
...
{
	return (g_aKeyState[nKey] & 0x80) ? true: false;	// ワーニングをとるため
}
